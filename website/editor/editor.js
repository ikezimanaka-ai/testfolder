(() => {
	...
	function ensureSvgEditor() {
		...
		svgEditorReady = import('./editor/Editor.js').then(({ default: Editor }) => {
			...
			return svgEditor.init();
		});
		return svgEditorReady;
	}
	function syncSvgEditor() {
		const tab = getSvgTab();
		if (!tab || !svgEditor?.isReady || !svgEditor.svgCanvas?.getSvgString) return;
	...
	function loadSvgTab(tab) {
		if (!tab) return;
		ensureSvgEditor().then(() => { if (svgEditor?.loadSvgString) svgEditor.loadSvgString(svgSource(tab)); });
	}
	...
	function svgMarkup(tab) {
		const body = tab.elements.map((item) => {
			if (item.image) return `<image href="${item.image}" x="${item.x}" y="${item.y}" width="${item.width}" height="${item.height}" preserveAspectRatio="none"/>`;
			if (item.type === 'circle') return `<circle cx="${item.cx}" cy="${item.cy}" r="${item.r}" fill="${item.fill}"/>`;
			if (item.type === 'polygon') return `<polygon points="${item.points}" fill="${item.fill}"/>`;
			return `<rect x="${item.x}" y="${item.y}" width="${item.width}" height="${item.height}" rx="${item.rx || 0}" fill="${item.fill}"/>`;
		}).join('');
		return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${tab.width} ${tab.height}" width="${tab.width}" height="${tab.height}">${body}</svg>`;
	}
	function compileSvg(object) {
		const tab = getSvgTab(object); if (!tab) return;
		if (object === selected) syncSvgEditor();
	...
	function runCode() { clearRuntime(); syncPrograms(); syncEngine(); compiledCanvas = { width: Math.max(1, Number($('canvas-width').value) || 960), height: Math.max(1, Number($('canvas-height').value) || 540) }; compiledRuntime = { engineInitial: getValue(engineInitialEditor, $('engine-initial-program')), engineFrame: getValue(engineFrameEditor, $('engine-frame-program')), templates: templates.map((object) => object.getInfo()) }; engine.startupPrograms = compiledRuntime.engineInitial.trim() ? [compiledRuntime.engineInitial] : []; engine.framePrograms = compiledRuntime.engineFrame.trim() ? [compiledRuntime.engineFrame] : []; engine.compileTemplateSpecs(compiledRuntime.templates); setCanvasSize(true); try { engine.runStartupPrograms(); startupExecuted = true; $('engine-error').textContent = ''; $('runtime-state').textContent = 'RUNNING'; } catch (error) { startupExecuted = false; compiledRuntime = null; engine.clearCompiledTemplateSpecs(); compiledCanvas = null; $('engine-error').textContent = error.message; } }
	...
	$('add-svg-tab').onclick = () => selected && addSvgTab(); $('import-svg').onclick = () => $('svg-file').click(); $('import-image').onclick = () => $('image-file').click(); $('svg-file').onchange = (event) => event.target.files[0] && importSvg(event.target.files[0]); $('image-file').onchange = (event) => event.target.files[0] && importImage(event.target.files[0]);
