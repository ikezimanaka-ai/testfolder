...
function runPrograms(dt) { if (!startupExecuted) return; const scope = Object.fromEntries(engine.displays.map(layer => [layer.name, layer])); scope.INPUT = engine.public.INPUT; for (const source of engine.framePrograms || []) { try { new Function('engine','dt','INPUT','scope',`with (scope) { ${source}\n }`)(engine, dt, engine.public.INPUT, scope); } catch (error) { $('#engine-settings-error').textContent = error.message; } } for (const display of engine.displays) for (const object of display.listObjects()) for (const source of object.programs) { try { new Function('self','display','engine','dt','INPUT','scope',`with (scope) { ${source}\n }`)(object, display, engine, dt, engine.public.INPUT, scope); } catch (error) { $('#program-error').textContent = error.message; } } }
